...
const {REMOVE_ANNOTATION, CONFIRM_REMOVE_ANNOTATION, CANCEL_REMOVE_ANNOTATION, CLOSE_ANNOTATIONS,
    ...
    ADD_TEXT, HIDE_TEXT, SHOW_TEXT_AREA, SAVE_TEXT} = require('../actions/annotations');
    ...
    switch (action.type) {
            ...
            return assign({}, state, {
                editing: {
                    ...
                    properties: assign({}, state.editing.properties, {
                            textValues: (state.editing.properties.textValues || []).concat([action.value]),
                            textGeometriesIndexes: (state.editing.properties.textGeometriesIndexes || []).concat([state.editing.geometry && state.editing.geometry.geometries && state.editing.geometry.geometries.length - 1 || 0])
                        })
                },
                drawingText: {
                    ...state.drawingText,
        ...
        case HIDE_TEXT:
            return assign({}, state, {
                ...
                editing: assign({},
                    state.editing, { geometry: assign({},
                        state.editing.geometry,
                        {geometries: state.editing.geometry.geometries.length === 1 ? [] : slice(state.editing.geometry.geometries)}
                    )}
                )
            });
        case SHOW_TEXT_AREA:
        ...
        case UPDATE_ANNOTATION_GEOMETRY: {
            ...
            if (action.geometry.type === "GeometryCollection") {
                stylerType = getAvailableStyler(action.geometry).indexOf(state.stylerType) !== -1 ? state.stylerType : head(getAvailableStyler(action.geometry));
            } else {
                stylerType = head(getAvailableStyler(action.geometry));
            }
