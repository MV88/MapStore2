...
function annotations(state = { validationErrors: {} }, action) {
    switch (action.type) {
        ...
        case SAVE_TEXT: {
            const textValues = (state.editing.properties.textValues || ["new"]).map(v => {
                return v === "" ? action.value : v;
            });
            const textGeometriesIndexes = (state.editing.properties.textGeometriesIndexes || [state.editing.geometry.geometries.length - 1]).map((v, i) => {
        ...
        case UPDATE_ANNOTATION_GEOMETRY: {
            let stylerType;
            let availableStyler = getAvailableStyler(convertGeoJSONToInternalModel(action.geometry, action.textChanged ? state.editing.properties.textValues || ["v"] : [] ));
