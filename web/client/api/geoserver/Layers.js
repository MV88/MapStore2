...
* @name api.geoserver
*/
export const getLayer = (geoserverBaseUrl, layerName, options) => {
...
*/
export const removeStyles = ({baseUrl, layerName, styles = [], options = {}}) => {
...
* @return {object} geoserver layer object with updated styles
*/
export const updateAvailableStyles = ({baseUrl, layerName, styles = [], options = {}}) => {
    const { name, workspace } = getNameParts(layerName);
    const url = `${baseUrl}rest/${workspace && `workspaces/${workspace}/` || ''}layers/${name}.json`;
    return axios.get(url, options)
        .then(({data}) => {
...
* @return {object} geoserver layer object with updated styles
*/
export const updateDefaultStyle = ({baseUrl, layerName, styleName, options = {}}) => {
    const { name, workspace } = getNameParts(layerName);
    const url = `${baseUrl}rest/${workspace && `workspaces/${workspace}/` || ''}layers/${name}.json`;
    return axios.get(url, options)
        .then(({ data }) => {
