...
*/
    ...
    addFeatures = ({features, drawMethod, options}) => {
        features.forEach((f) => {
            ...
            if (geometry.geometry && geometry.geometry.type !== "GeometryCollection") {
                geometry = reprojectGeoJson(geometry.featureProjection, this.props.map.getView().getProjection().getCode()).geometry;
            }
        ...
        if (features.length === 0 && (options.editEnabled || options.drawEnabled)) {
            ...
            if (features[0].geometry && features[0].geometry.type === "GeometryCollection" ) {
    ...
    handleDrawAndEdit = (drawMethod, startingPoint, maxPoints, style) => {
        ...
        //     let previousTexts = this.drawLayer.getSource().getFeatures()[0].get("textGeometriesIndexes") || [];
            ...
            const newFeatures = isSimpleGeomType(this.props.drawMethod) ? this.props.features.concat([{...feature, properties: this.props.features[0] && this.props.features[0].properties}]) : [{...feature, properties: this.props.features[0] && this.props.features[0].properties}];
            if (this.props.options.stopAfterDrawing) {
    ...
    drawPropertiesForGeometryType = (geometryType, maxPoints, source, style) => {
        ...
        let drawBaseProps = {
            ...
            style: VectorStyle.getStyle({style, fcoll}, true) || new ol.style.Style({
