...
export const normalizeExtent = (bounds, projection) => {
 ...
 */
export const getProjectedBBox = (center, resolution, rotation = 0, size) => {
 ...
 */
export const reprojectGeoJson = (geojson, fromParam = "EPSG:4326", toParam = "EPSG:4326") => {
    let from = fromParam;
    let to = toParam;
...
export const lineIntersectPolygon = (linestring, polygon) => {
 ...
 */
export const reprojectBbox = (bbox, source, dest, normalize = true) => {
    let points;
    if (isArray(bbox)) {
        points = {
            sw: [bbox[0], bbox[1]],
    ...
    for (let p in points) {
        if (points.hasOwnProperty(p)) {
            const projected = reproject(points[p], source, dest, normalize);
            if (projected) {
                let {x, y} = projected;
                projPoints.push(x);
                projPoints.push(y);
...
export const calculateAzimuth = function(p1, p2, pj) {
    var p1proj = reproject(p1, pj, 'EPSG:4326');
    var p2proj = reproject(p2, pj, 'EPSG:4326');
 ...
 */
export const getGeoJSONExtent = (geoJSON) => {
...
export const transformArcsToLine = (coordinates, npoints = 100) => {
    if (coordinates.length <= npoints) {
        ...
        return [head(coordinates)].concat(transformArcsToLine(slice(coordinates, npoints)));
    }
    return [];
};
...
export const isInsideVisibleArea = (coords, map, layout = {}, resolution = 0) => {
    ...
    const normalizedCoords = getNormalizedLatLon(coords);
    const reprojectedCoords = reproject([normalizedCoords.lng, normalizedCoords.lat], 'EPSG:4326', map.projection);
    if (!map.bbox) {
        return false;
    }
    const bbox = reprojectBbox(map.bbox.bounds, map.bbox.crs, map.projection);
    ...
    const splittedView = getViewportGeometry(visibleExtent, map.projection);
...
export const centerToVisibleArea = (center, map, layout = {}, resolution = 0) => {
    ...
    const normalizedCoords = getNormalizedLatLon(center);
    const reprojectedCoords = reproject([normalizedCoords.lng, normalizedCoords.lat], 'EPSG:4326', map.projection);
    ...
    const splittedView = getViewportGeometry(mapExtent, map.projection);
