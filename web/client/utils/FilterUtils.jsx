    ...
    if (fields) {
        fields.forEach((field) => {
            ...
            case "string":
                ...
                break;
            case "boolean":
                fieldFilter = cqlBooleanField(field.attribute, field.operator, field.value);
                break;
            case "list":
...
export const processCQLSimpleFilterField = function(field) {
    ...
    case "string":
        ...
        break;
    case "boolean":
        strFilter = cqlBooleanField(field.attribute, field.operator, field.values);
        break;
    case "list": {
...
export const processCQLFilterGroup = function(root, objFilter) {
    ...
    let cql = processCQLFilterFields(fixedRoot, objFilter);

    let subGroups = findSubGroups(fixedRoot, objFilter.groupFields);
    if (subGroups.length > 0) {
        const subGroupCql = subGroups
            .map((subGroup) => (fixedRoot.negateAll ? "NOT (" : "(") + processCQLFilterGroup(subGroup, objFilter) + ")")
...
export const processCQLSpatialFilter = function(objFilter) {
    ...
    if (objFilter.spatialField.collectGeometries && objFilter.spatialField.collectGeometries.queryCollection) {
        ...
        cql += crs + getCQLGeometryElement(objFilter.spatialField.geometry.coordinates, objFilter.spatialField.geometry.type);
    }

    return cql + ")";
};

export let getCrossLayerCqlFilter;
export const toCQLFilter = function(json) {
    ...
    if (objFilter.crossLayerFilter) {
        ...
        if (operation && attribute && queryCollection) {
            ...
            const cqlFilter = getCrossLayerCqlFilter(crossLayerFilter)
...
getCrossLayerCqlFilter = crossLayerFilter => get(crossLayerFilter, 'collectGeometries.queryCollection.cqlFilter')
    || (get(crossLayerFilter, 'collectGeometries.queryCollection.filterFields') || []).length > 0
        ...
        && toCQLFilter(crossLayerFilter.collectGeometries.queryCollection)
    || "INCLUDE";
...
export const processOGCFilterGroup = function(root, objFilter, nsplaceholder) {
    let ogc = processOGCFilterFields(root, objFilter, nsplaceholder);

    let subGroups = findSubGroups(root, objFilter.groupFields);
    if (subGroups.length > 0) {
        subGroups.forEach((subGroup) => {
            ogc += processOGCFilterGroup(subGroup, objFilter, nsplaceholder);
        });
    }
    if (ogc !== "") {
...
export const processOGCSpatialFilter = function(version, objFilter, nsplaceholder) {
    ...
    if (objFilter.spatialField.collectGeometries) {
        return processOGCCrossLayerFilter(objFilter.spatialField);
    }
    let ogc =
        propertyTagReference[nsplaceholder].startTag +
            objFilter.spatialField.attribute +
...
export const toOGCFilterParts = function(objFilter, versionOGC, nsplaceholder) {
    ...
    if (objFilter.filterFields && objFilter.filterFields.length > 0) {
        if (objFilter.groupFields && objFilter.groupFields.length > 0) {
            ...
            attributeFilter = processOGCFilterFields(null, objFilter, nsplaceholder);
        }
        if (attributeFilter !== "") {
            filters.push(attributeFilter);
        }
    ...
    if (objFilter.spatialField && objFilter.spatialField.geometry && objFilter.spatialField.operation) {
        if (objFilter.spatialField.operation === 'BBOX' && isArray(objFilter.spatialField.geometry && objFilter.spatialField.geometry.extent[0])) {
            ...
            const bBoxFilter = objFilter.spatialField.geometry.extent.reduce((a, extent) => {
                ...
                return a + processOGCSpatialFilter(versionOGC, filter, nsplaceholder);
            }, '');

            spatialFilter = ogcLogicalOperators[OP](nsplaceholder, bBoxFilter);

        } else {
            spatialFilter = processOGCSpatialFilter(versionOGC, objFilter, nsplaceholder);
        }
    ...
    if (objFilter.crossLayerFilter && objFilter.crossLayerFilter.operation) {
        ...
        if (Array.isArray()) {
            crossLayerFilter.forEach( f => filters.push(processOGCCrossLayerFilter(f, nsplaceholder)));
        } else {
            filters.push(processOGCCrossLayerFilter(crossLayerFilter, nsplaceholder));
        }
    }
...
export const toOGCFilter = function(ftName, json, version, sortOptions = null, hits = false, format = null, propertyNames = null, srsName = "EPSG:4326") {
    ...
    let ogcFilter = getGetFeatureBase(versionOGC, objFilter.pagination, hits, format, json && json.options);

    let filters = toOGCFilterParts(objFilter, versionOGC, nsplaceholder);
    let filter = "";
...
export const processOGCGeometry = (v, geom) => processOGCGeometryHandler(wfsToGmlVersion(v), geom);
...
export const getSLD = function(ftName, json, version) {
    let filter = toOGCFilter(ftName, json, version);
    let sIdx = filter.search( `<${version === "2.0" ? "fes" : "ogc"}:Filter>`);
    if (sIdx !== -1) {
...
export const getWFSFilterData = (filterObj, options) => {
    ...
    if (typeof filterObj === 'string') {
        ...
        data = filterObj.filterType === "OGC"
            ? toOGCFilter(filterObj.featureTypeName, {...filterObj, options}, filterObj.ogcVersion, filterObj.sortOptions, filterObj.hits)
            : toCQLFilter(filterObj);
    }
    return data;
};
...
export const composeAttributeFilters = (filters, logic = "AND") => {
    const rootGroup = {
        id: new Date().getTime(),
        index: 0,
        logic
...
export const normalizeFilterCQL = (filter, nativeCrs) => {
    if (filter && filter.spatialField && filter.spatialField.geometry && filter.spatialField.geometry.coordinates && filter.spatialField.geometry.coordinates[0] && (filter.spatialField.projection || "EPSG:3857") !== nativeCrs) {
        ...
        return reprojectFilterInNativeCrs(filter, nativeCrs);
    }
    return filter;
};
...
const FilterUtils = {
    ...
    getGetFeatureBase,
    getGmlLineStringElement,
    getGmlPointElement,
    getGmlPolygonElement,
    getOgcAllPropertyValue,
    getSLD,
    getWFSFilterData,
