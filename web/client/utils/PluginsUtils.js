...
import { connect as connectLib } from 'react-redux';
import { combineReducers as combineReducersLib } from 'redux';
import { combineEpics as combineEpicsLib } from 'redux-observable';
...
export const getEpics = (plugins) => Object.keys(plugins).map((name) => plugins[name].epics)
    .reduce((previous, current) => assign({}, previous, current), {});
...
export const combineReducers = (plugins, reducers) => {
    ...
    return combineReducersLib(assign({}, reducers, pluginsReducers));
};
/**
...
export const combineEpics = (plugins, epics = {}, epicWrapper = defaultEpicWrapper) => {
    ...
    return combineEpicsLib( ...Object.keys(pluginEpics).map(k => pluginEpics[k]).map(epicWrapper));
};
...
export const connect = (mapStateToProps, mapDispatchToProps, mergeProps, options) => {
    return connectLib(mapStateToProps, mapDispatchToProps, mergeProps || pluginsMergeProps, options);
};
/**
