...
export const initialReorderLayers = (groups, allLayers) => {
...
export const reorderLayers = (groups, allLayers) => {
    return initialReorderLayers(groups, allLayers);
};
export const createGroup = (groupId, groupName, layers, addLayers) => {
    return assign({}, {
        id: groupId,
...
export const getElevationDimension = (dimensions = []) => {
...
export const addBaseParams = (url, params) => {
...
export const getGroupByName = (groupName, groups) => {
    ...
    return result || groups.reduce((prev, g) => prev || !!g.nodes && getGroupByName(groupName, g.nodes), undefined);
};
 ...
 */
export const normalizeLayer = (layer) => layer.id ? layer : { ...layer, id: getLayerId(layer) };
/**
...
export const normalizeMap = (rawMap = {}) =>
    [
        (map) => (map.layers || []).filter(({ id } = {}) => !id).length > 0 ? {...map, layers: (map.layers || []).map(l => normalizeLayer(l))} : map,
...
export const getLayersByGroup = (configLayers) => {
    let i = 0;
...
// TODO TEST NEXT FUNC, IF conversion is good
    ...
    if (mapState && isArray(mapState.layers)) {
        ...
        if (isArray(mapState.groups)) {
            groups = mapState.groups.reduce((g, group) => {
                let newGroups = g;
                if (group.title) {
                    newGroups = deepChange(newGroups, group.id, 'title', group.title);
                }
                newGroups = deepChange(newGroups, group.id, 'expanded', group.expanded);
                return newGroups;
            }, [].concat(groups));
        }
...
export const findGeoServerName = ({url, regexRule}) => {
    const regex = regexRule || getRegGeoserverRule();
...
export const invalidateUnsupportedLayer = (layer, maptype) => {
    return isSupportedLayerForMapType(layer, maptype) ? checkInvalidParam(layer) : assign({}, layer, {invalid: true});
};
/**
...
export const isSupportedLayer = (layer, maptype) => {
    return !!isSupportedLayerForMapType(layer, maptype);
};
...
const LayersUtils = {
    ...
    excludeGoogleBackground,
    creditsToAttribution,
    formatCapabitiliesOptions,
    getWpsUrl
};
