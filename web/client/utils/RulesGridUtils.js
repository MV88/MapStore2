...
export const getPageIdx = (i, size) => Math.floor(i / size);
export const getRow = (i, pages = {}, size) => {
...
export const getPagesToLoad = (startPage, endPage, pages) => {
    let needPages = [];
 ...
 *
 */
export const updatePages = (newPages = {}, {startPage, endPage, pages: oldPages}, maxStoredPages = 5) => {
...
export const flattenPages = (pages = {}) => Object.keys(pages).reduce((rows, key) => rows.concat((pages[key] || [])), []);
export const getOffsetFromTop = (row, rows) => rows.indexOf(row);
export const getClosestRows = (row, rows) => {
