...
export const rgbToHex = ( r, g, b ) => {
    if ( r instanceof Array ) { return rgbToHex( r[0], r[1], r[2] ); }
    return "#" + decToHex( r ) + decToHex( g ) + decToHex( b );
};
export const realToDec = n => {
    return Math.min( 255, Math.round( n * 256 ) );
};
export const rgbToHsv = ( rr, gg, bb ) => {
    if ( rr instanceof Array ) { return rgbToHsv( rr[0], rr[1], rr[2] ); }
    let r = rr / 255;
    let g = gg / 255;
    let b = bb / 255;
    let min;
 ...
 */
export const distributedColorsHEX = (total) => {
...
export const hsvToRgb = ( h, s, v ) => {
    if ( h instanceof Array ) { return hsvToRgb( h[0], h[1], h[2] ); }
    let r;
    let g;
    let b;
    ...
    return [realToDec( r ), realToDec( g ), realToDec( b )];
};
 ...
 */
export const sameToneRangeColors = (bb, range, total, options) => {
    // if base is a string i suppose is an hex color
    let base = bb;
    if (isNaN(parseFloat(bb))) {
        let hsvbase = hexToHsv(bb);
        base = hsvbase[0];
    }
    ...
    for (let x = 0; x < total; x++) {
        ...
        r.push(hsvToHex(h, s, v, x));
    }
    return r;
};

export const colorToHexStr = (color = 'red') => tinycolor(color).toHexString();
/**
...
*/
export const colorToRgbaStr = (color, alpha, defaultColor) => {
